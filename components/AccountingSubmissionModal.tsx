...
interface AccountingSubmissionModalProps {
    isOpen: boolean;
...
// Default translations for accounting submission modal
const DEFAULT_TRANSLATIONS = {
    confirmTitle: 'Send to Accounting?',
    userName: 'User',
    month: 'Month',
    studies: 'Number of Studies',
    amount: 'Total Amount',
    confirmMessage: 'Please review the details before sending to accounting. This action will finalize the report for payment processing.',
    yes: 'Yes, Send',
    no: 'Cancel',
    submitting: 'Sending to Accounting',
    progress: 'Processing',
    processingMessage: 'Please wait while the report is being sent to accounting...',
    successTitle: 'Sent to Accounting Successfully',
    successMessage: 'The report has been sent to accounting and will be processed for payment.',
    close: 'Close',
};

const AccountingSubmissionModal: React.FC<AccountingSubmissionModalProps> = ({
    isOpen,
    reportUserName,
    month,
    year,
    ...
    const translate = (key: string, defaultValue: string) => {
        try {
            const value = t(`reports.accounting.${key}`);
            return value === `reports.accounting.${key}` ? defaultValue : value;
        } catch {
            return defaultValue;
        }
    };
                    ...
                    <>
                        <div className="p-6">
                            ...
                            <div className="space-y-3 mb-6">
                                <div className="flex justify-between items-center p-3 bg-gray-900/50 rounded-lg">
                                    <span className="text-gray-400">{translate('userName', DEFAULT_TRANSLATIONS.userName)}:</span>
                                    <span className="text-white font-semibold">{reportUserName}</span>
                                </div>
                                <div className="flex justify-between items-center p-3 bg-gray-900/50 rounded-lg">
                                ...
                                <div className="flex justify-between items-center p-3 bg-gray-900/50 rounded-lg">
                                    ...
                                    <span className="text-green-400 font-semibold">{totalAmount.toFixed(2)} PLN</span>
                                </div>
                            </div>
                        ...
                        <div className="p-4 border-t border-gray-700 bg-gray-900/50 grid grid-cols-2 gap-3">
                            ...
                            <button
                                ...
                                className="w-full px-4 py-2 bg-blue-600 hover:bg-blue-700 rounded-md text-white font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500"
                            >
                ...
                {step === 'completed' && (
                    <>
                        <div className="p-6 text-center">
                            <div className="mx-auto flex items-center justify-center h-16 w-16 rounded-full bg-blue-900/50 mb-4">
...
export default AccountingSubmissionModal;
