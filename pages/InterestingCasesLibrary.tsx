...
import { ArrowLeftIcon, PencilIcon, TrashIcon, CopyIcon, CheckIcon } from '../components/Icons';
            ...
            <div className="bg-gray-800 border border-gray-700 rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col animate-fade-in" onClick={e => e.stopPropagation()}>
                <header className="p-4 border-b border-gray-700 flex justify-between items-center">
                    <div>
                        ...
                        <p className="text-sm" style={{ color: currentTheme.colors.textSecondary }}>{t('library.studyNumber')}: {caseData.studyNumber}</p>
                    </div>
                    <div className="flex items-center gap-2">
                         <button onClick={() => onEdit(caseData)} style={{ color: currentTheme.colors.accentPrimary }} className="p-2 rounded-full transition-colors" onMouseEnter={(e) => e.currentTarget.style.backgroundColor = currentTheme.colors.bgTertiary} onMouseLeave={(e) => e.currentTarget.style.backgroundColor = 'transparent'} title={t('templates.edit')}><PencilIcon className="h-5 w-5"/></button>
                         <button onClick={() => onDelete(caseData.id)} style={{ color: currentTheme.colors.accentError }} className="p-2 rounded-full transition-colors" onMouseEnter={(e) => e.currentTarget.style.backgroundColor = currentTheme.colors.bgTertiary} onMouseLeave={(e) => e.currentTarget.style.backgroundColor = 'transparent'} title={t('templates.delete')}><TrashIcon className="h-5 w-5"/></button>
                    </div>
                </header>
                ...
                <footer className="p-4 border-t border-gray-700 flex justify-end">
                     <button onClick={onClose} style={{ backgroundColor: currentTheme.colors.buttonSecondary, color: currentTheme.colors.textPrimary }} className="px-4 py-2 rounded-md font-semibold transition-colors" onMouseEnter={(e) => e.currentTarget.style.backgroundColor = currentTheme.colors.bgHover} onMouseLeave={(e) => e.currentTarget.style.backgroundColor = currentTheme.colors.buttonSecondary}>{t('library.close')}</button>
                </footer>
            </div>
        </div>
    );
...
const InterestingCasesLibrary: React.FC = () => {
  const { t } = useTranslations();
  const { setView, caseModal, setConfirmationState, closeConfirmation } = useApp();
  const { interestingCases, moveCaseToTrash } = useSettings();
  ...
  useEffect(() => {
    if (selectedCase && !interestingCases.find(c => c.id === selectedCase.id)) {
      setSelectedCase(null);
    }
  }, [interestingCases, selectedCase]);

  const allTags = useMemo(() => {
    const tags = new Set<string>();
    interestingCases.forEach(c => c.tags.forEach(tag => tags.add(tag)));
  ...
  }, [interestingCases, searchTerm, selectedTags]);
  ...
  const handleEdit = (caseData: InterestingCase) => {
      setSelectedCase(null); // Close detail modal first
      caseModal.setEditingCase(caseData);
  ...
  const handleDelete = (caseId: string) => {
    setConfirmationState({
      ...
      message: t('library.deleteConfirm', { studyNumber: selectedCase?.studyNumber }),
      onConfirm: () => {
        moveCaseToTrash(caseId);
        closeConfirmation();
      }
    });
  };
    ...
    <main className="bg-gray-900 text-gray-200 min-h-screen p-4 font-sans">
      <div className="w-full max-w-7xl mx-auto pb-12">
        <header className="flex items-center mb-6">
          <button onClick={() => setView('main')} className="mr-4 p-2 text-gray-400 hover:text-white hover:bg-gray-700 rounded-full transition-colors" aria-label={t('settings.back')}>
            <ArrowLeftIcon />
          </button>
      ...
      {selectedCase && <CaseDetailModal caseData={selectedCase} onClose={() => setSelectedCase(null)} onEdit={handleEdit} onDelete={handleDelete} />}
    </main>
  );
};
